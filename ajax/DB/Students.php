...
class Students extends Database {
  ...
  private function filterTime(){
    switch($this->filters->time){
      case self::TIME_FUTURE:
        $this->request->where('s.Tstart < "' . strtotime('now') . '"');
        break;
        
      case self::TIME_PAST:
        $this->request->where('s.Tend > "' . strtotime('now') . '"');
        break;
    }
  }
  ...
  private function readAll(){
    ...
    $sessions = $this->request
      ->again()
      ->select (array('sessions' => 's.id as id, Tstart as timeStart, Tend as timeStop'))
      ->from   ('sessions as s')
      ->orderBy('Tend ASC')
      ->get    ();
...
class student extends table {
  ...
  public function __construct(){
    ...
    switch(count($parameters)){
      ...
      case 1:
        $this->id        = $parameters[0]['id'];
        $this->timeStart = $parameters[0]['timeStart'];
        $this->timeStop  = $parameters[0]['timeStop'];
        $this->timeStop  = $parameters[0]['timeStop'];
        $this->timeStop  = $parameters[0]['timeStop'];
        break;
      ...
      case 3:
        ...
        $this->timeStart = $parameters[1];
        $this->timeStop  = $parameters[2];
        break;
    }
...
class Tests {
  ...
  static function create(){
    $test = new Sessions();
    $result = $test
      ->setTimeStart(new DateTime('today'))
      ->setTimeStop(new DateTime('tomorrow'))
      ->create();
    debug($result);
  }
}
